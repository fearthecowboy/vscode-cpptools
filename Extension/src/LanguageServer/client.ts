...
export class DefaultClient implements Client {
        ...
        try {
            ...
            if (index === paths.length - 3) {
                ...
                if (index < configProvidersIndex && configProviders) {
                    ...
                    void this.configuration.setCompileCommands(this.compileCommandsPaths[index - configProvidersIndex - 1]).catch(logAndReturn.undefined);
                    return ui.ShowConfigureIntelliSenseButton(false, this);
                } else {
    ...
    public async onDidChangeSettings(event: vscode.ConfigurationChangeEvent): Promise<Record<string, string>> {
        ...
        if (Object.keys(changedSettings).length > 0) {
            if (this === defaultClient) {
                ...
                if (changedSettings["caseSensitiveFileSupport"] && isWindows) {
                    void util.promptForReloadWindowDueToSettingsChange().catch(logAndReturn.undefined);
                }
    ...
    public async handleConfigStatusOrPrompt(sender?: string): Promise<void> {
        ...
        if (statusBarIndicatorEnabled) {
            ...
            await ui.ShowConfigureIntelliSenseButton(this.showConfigureIntelliSenseButton, this);
